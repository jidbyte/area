    ...
    >
      ...
      >
        {items.map((name, i) => (
          <Link
            key={`${name}-${i}`}
            href={`/${shopSlug}/category/${encodeURIComponent(name)}`}
            aria-hidden={i >= categories.length}
            tabIndex={i >= categories.length ? -1 : undefined}
            className="shrink-0 rounded-md bg-ink/10 px-5 py-2 text-sm font-medium text-ink transition-colors hover:bg-neutral hover:text-white"
          >
            {name}
          </Link>
        ))}
