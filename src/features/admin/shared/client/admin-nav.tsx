...
import { navItems } from "./admin-head";
import { cn } from "@/shared/lib/utils";
import Link from "next/link";

export function AdminNavigationTabs({ storeSlug }: { storeSlug: string }) {
  const pathname = usePathname();
  const items = navItems(storeSlug);
  const dashboardHref = `/${storeSlug}/dashboard`;

  function isActive(pathname: string, href: string) {
    return href === dashboardHref ? pathname === dashboardHref : pathname.startsWith(href);
  }

  const activeHref = items.find((item) => isActive(pathname, item.href))?.href;

  return (
    <div className="border-b border-muted/40 hidden md:block bg-muted/30">
          ...
          <TabsList variant="line" className="flex gap-6">
            {items.map((item) => (
              <TabsTrigger
                ...
                asChild
                className={cn(
                  "font-semibold border-none text-neutral px-0 hover:text-ink",
