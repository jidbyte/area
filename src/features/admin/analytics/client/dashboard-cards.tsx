      ...
      <CardContent className="space-y-1 -mt-4">
        {products.map((p) => {
          ...
          return (
            ...
            >
              <div className="flex size-8 shrink-0 items-center justify-center rounded-md bg-muted/40 text-neutral">
      ...
      <CardContent className="space-y-2.5 -mt-4">
        {sales.length === 0 ? (
          ...
          sales.map((s) => (
            <Link
              key={s.id}
              href={`/${shopSlug}/transactions/sales/${s.id}`}
              className="flex items-center gap-3 rounded-lg p-2 -mx-2 transition-colors bg-accent/20 hover:bg-accent/40"
            >
              <div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-neutral text-xs font-semibold text-surface">
        ...
        {products.map((p, i) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-neutral text-semibold">{p.quantity} sold</p>
            </div>
