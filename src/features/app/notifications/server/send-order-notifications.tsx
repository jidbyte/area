...
export async function sendOrderNotifications(saleId: string): Promise<void> {
  ...
  if (buyerEmail && shop.emailNotificationsEnabled) {
    try {
  ...
  if (shop.email && shop.emailNotificationsEnabled) {
    try {
      const resend = getResendClient();
      await resend.emails.send({
        from: fromAddressForShop(shop.name),
  ...
  if (buyerPhone && shop.whatsappNotificationsEnabled) {
    try {
  ...
  if (shop.phone && shop.whatsappNotificationsEnabled) {
    try {
