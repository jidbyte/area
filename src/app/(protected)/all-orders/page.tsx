      ...
      <div className="mx-auto max-w-3xl p-4 md:px-12 py-10">
        ...
        {orders.length === 0 ? (
          ...
          <div className="mt-8 space-y-6">
            {orders.map((order) => (
              <div key={order.id} className="rounded-lg border border-muted/40 p-5">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-muted/40 pb-3">
                  <div>
                    <p className="font-medium text-ink">
                      {order.shop ? (
                        <Link href={`/stores/${order.shop.slug}`} className="hover:underline">
                          {order.shop.name}
                        </Link>
                      ) : (
                        "Unknown store"
                  ...
                  <div className="text-right">
                    <p className="text-sm font-semibold text-ink">
                      {formatPrice(order.totalAmount, order.shop?.currency ?? "USD")}
                    </p>
                    <p className="text-xs text-neutral">
                  ...
                  {order.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {item.productId && order.shop && order.paymentStatus !== "cancelled" && (
